@if(count($expertises))
    ...
    @if($expertises->lastPage() > $expertises->currentPage())
        <div class="load">
            <button data-search="{{ $q }}" data-filter_type="{{ $filter_type }}" data-target="{{ $expertises->nextPageUrl() }}" data-sort_by="{{ $sort_by }}" class="btn btn-primary btn-lg loadmore-search-expertise">Load more</button>
        </div>
    @endif
@else
    No result found
